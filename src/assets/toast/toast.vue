<template>
  <transition v-if="showToast" name="fade">
    <div class="miz-toast" :style="computedStyle">
      ...
      <span v-html="msg" :class="{'center_span': toastConfig.textAlignCenter}"></span>
    </div>
  </transition>
</template>
<script>
  ...
  computed: {
    positionStyle() {
      ...
      if (/^bottom|^top/i.test(pos)) {
        ...
        if (!kv[1]) {
          // 默认值
          kv[1] = (180 / 19.2).toFixed(4) + "vw;";
        } else {
          // 纯数字或者px结尾的px值需转换
          if (/^\d+$/.test(kv[1]) || /px$/i.test(kv[1])) {
            kv[1] = (parseInt(kv[1]) / 19.2).toFixed(4) + "vw;";
          }
        }
        return `${kv[0]}:${kv[1]}`;
      } else {
    ...
    fontStyle() {
      return `font-size:${(this.config.fontSize / 19.2).toFixed(4)}vw;color:${
        this.config.color
      };line-height:${this.config.lineHeight};`;
    },
    boxStyle() {
      return `width:${this.config.width};padding:${(
        this.config.columnPadding / 19.2
      ).toFixed(4)}vw ${(
        this.config.horizontalPadding / 19.2
      ).toFixed(4)}vw;border-radius:${(this.config.borderRadius / 19.2).toFixed(
        4
      )}vw;`;
    },
    bgStyle() {
